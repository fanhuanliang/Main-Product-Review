  ...
  constructor(props) {
    ...
    hiddenParagraph() {
      if (this.state.hidingParagraph === '60px') {
        this.setState({ hidingParagraph: 'auto',
        displayParagraph: 'Show Less'
       });
      } else if (this.state.hidingParagraph === 'auto') {
        this.setState({ hidingParagraph: '60px',
        displayParagraph: 'Read more' });
      }
    }
    ...
    render(){
      return (
        <div className='paragraph'>
          ...
          <div>
            <button className='paragraph-button' onClick={() => { this.hiddenParagraph(); }}>{this.state.displayParagraph}</button>
          </div>
        </div>
      );
    }
